...
for o in range(5, 101, 5):
	cur.execute("""
	    SELECT HOUSEID, PERSONID, TDAYDATE, VEHID, TRPMILES 
	    FROM dayv2pub
	...
	for result in results:
		mpgmap[result[1]] = result[0]

	for i in out:
		if i[4] < o:
			numer = numer + daymap[i[2]] * i[4] * mpgmap[i[3]]
		denom = denom + daymap[i[2]] * i[4]

	ans = (numer/denom)
	print "Miles per gallon of " + str(o) + " miles is: " +  "{0:.2f}".format(ans)
