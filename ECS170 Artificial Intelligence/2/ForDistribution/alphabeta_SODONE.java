 ...
 */
public class alphabeta_SODONE extends AIModule
{
	...
	private int maxDepth = 6;		//maximum depth to search
	...
	private int weights[] = {1, 3, 5, 7, 6, 4, 2};
	...
	{		
		...
		while(!terminate)
		{
			alphabeta(gameCopy, 0, maxPlayer, -Integer.MAX_VALUE, Integer.MAX_VALUE, -1);
		}
		//System.out.println("Testing Ordering: ");
		//int moves[] = ordering(gameCopy);
	...
	public int[] ordering(GameStateModule gameCopy) {
		int posMoves = 0; //Number of possible moves we can make
		int[] tempMove = new int[7]; //holds temp moves 
		int height;		//height?
		int[] movesToMake; //moves to make for alpha beta
		int[] moveScore;	//a moves score
		int movesIndex = 0;
		
		//finds the columns that have valid moves
		for(int col = 0; col < boardWidth; col++)
		{
			...
			//if can't make a move, set that column to -1
		}
		movesToMake = new int[posMoves];
		moveScore = new int[posMoves];
		
		//assign scores to corresponding col with valid move and 
		//fill movesToMake array with valid move cols
		for(int i = 0; i < boardWidth; i++){
			if(gameCopy.canMakeMove(i))
			{
				moveScore[movesIndex] = weights[i];
				movesToMake[movesIndex] = i;
				movesIndex++;
			}
		}
		...
		{
			for(int k = (i+1); k < posMoves; k++)
			{
				if(moveScore[i] < moveScore[k])
				{
					tempScore = moveScore[i];
					moveScore[i] = moveScore[k];
					moveScore[k] = tempScore;

					tempCol = movesToMake[i];
					movesToMake[i] = movesToMake[k];
					movesToMake[k] = tempCol;
				}
			}	
		}
	...
	public int alphabeta(GameStateModule gameCopy, int curDepth, boolean minOrMaxPlayer, int alpha, int beta, int lastMove)
	{
		...
		else
		{
			...
			if(minOrMaxPlayer)
			{
				...
				int[] movesToMake =  ordering(gameCopy);
				//for(int i = 0; i < movesToMake.length; i++)
				//	System.out.print(movesToMake[i] + " ");
				//System.out.println();
				//for(int col = 0; col < boardWidth; col++)
				for(int col = 0; col < movesToMake.length; col++)
				{
					//If move is possible
					//if(gameCopy.canMakeMove(col));
					if(gameCopy.canMakeMove(movesToMake[col]))
					{
						//Make move in the copy of the game 
						gameCopy.makeMove(movesToMake[col]);
						...
						score = alphabeta(gameCopy, (curDepth + 1), minPlayer, alpha, beta, movesToMake[col]);
			...
			else
			{
				...
				for(int col = 0; col < movesToMake.length; col++)
				{
					...
					if(gameCopy.canMakeMove(movesToMake[col]))
					{
						...
						score = alphabeta(gameCopy, (curDepth + 1), maxPlayer, alpha, beta, movesToMake[col]);
						gameCopy.unMakeMove();
						if(score < bestValue)
						{
							bestValue = score;
