 ...
 public class AStar implements AIModule
{
		...
		private void expandPnts(Point curPnt, TerrainMap map)
	{
		...
		for(int i = 0; i < adjPoints.length; i++)
		{
			...
			double hn = getHeurExp(map, curPnt, adjPnt);
	...
	private Double getHeurDiv(final TerrainMap map, final Point pt1, final Point pt2)
	{
	...
	private Double getHeurExp(final TerrainMap map, final Point pt1, final Point pt2)
	{
		double dx = Math.abs(map.getEndPoint().x - pt1.x);
		double dy = Math.abs(map.getEndPoint().y - pt1.y);
