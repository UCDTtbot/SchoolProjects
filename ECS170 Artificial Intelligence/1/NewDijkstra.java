 ...
 public class NewDijkstra implements AIModule
{
	...
	private void expandPnts(Point curPnt, TerrainMap map)
	{
		...
		Point adjPnt;
		//double dx = Math.abs(map.getEndPoint().x - map.getStartPoint().x);
		//double dy = Math.abs(map.getEndPoint().y - map.getStartPoint().y);
		...
		for(int i = 0; i < adjPoints.length; i++)
		{
			...
			if((getPntCost(adjPnt) > gn && !explored.contains(adjPnt))) 
			{
					//add the adjNode (child) and its total cost to the costs map
				costMap.put(adjPnt, gn);
