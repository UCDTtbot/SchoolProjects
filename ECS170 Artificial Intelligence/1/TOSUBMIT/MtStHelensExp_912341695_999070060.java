 ...
 */
 public class MtStHelensExp_912341695_999070060 implements AIModule
{
	int check = 0;
    ...
    public List<Point> createPath(final TerrainMap map)
    {
		...
		{
		double dx = Math.abs(map.getEndPoint().x - curPnt.x);
		double dy = Math.abs(map.getEndPoint().y - curPnt.y);
		double sDx = Math.abs(map.getEndPoint().x - map.getStartPoint().x);
		double sDy = Math.abs(map.getEndPoint().y - map.getStartPoint().y);
		double pyth = Math.sqrt(dx * dx + dy *dy) ;
		double sPyth = Math.sqrt(sDx * sDx + sDy * sDy);
		...
		for(int i = 0; i < adjPoints.length; i++)
		{
			...
			if((getPntCost(adjPnt) > gn +  hn && !explored.contains(adjPnt))) 
			{
				if(pyth <= sPyth)
				{
					...
					if(getPntCost(adjPnt) - getPntCost(curPnt) > 0)
	...
	private Double getHeuristic(final TerrainMap map, final Point pt1, final Point pt2)
	{
		...
		final Point[] adjPoints = map.getNeighbors(pt1);
		Point adjPnt;
		...
		hMin = map.getTile(minPnt);
		
		if(Math.abs(hE - hC) == 0)
			D = 0;
		else if(Math.abs(hE - hC) < 1)
			D = Math.exp(Math.abs(hE-hC) / pyth);
		else
			D =  Math.exp(Math.abs(hE-hC) / pyth);
